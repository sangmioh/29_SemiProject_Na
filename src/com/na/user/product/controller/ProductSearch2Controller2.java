...
/**
 * Servlet implementation class ProductSearch2Controller2
 */
@WebServlet("/proSearch21.pr")
public class ProductSearch2Controller2 extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ProductSearch2Controller2() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ArrayList<Product> list = new ProductService().selectSearchList21(pi, s1, s2);
		
		request.setAttribute("list", list);
		request.setAttribute("pi", pi);
		request.setAttribute("s1", s1);
		request.setAttribute("s2", s2);
		request.getRequestDispatcher("views/user/product/proListSearched21.jsp").forward(request, response);
	}
