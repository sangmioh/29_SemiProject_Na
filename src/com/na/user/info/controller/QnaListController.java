...
/**
 * Servlet implementation class QnaListFormController
 */
@WebServlet("/qnalist.in")
public class QnaListController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		listCount = new InfoService().selectListCount5(userNo);
		...
		// 현재 세션으로부터 값을 뽑아오기
			...
			ArrayList<Qna> list = new InfoService().selectQnaList(pi,userNo);
			request.setAttribute("list", list);
			request.setAttribute("pi", pi);
			request.getRequestDispatcher("views/user/info/qnaList.jsp").forward(request, response);
