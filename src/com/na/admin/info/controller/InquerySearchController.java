...
/**
 * Servlet implementation class InquerySearchController
 */
@WebServlet("/inquerysearch.in")
public class InquerySearchController extends HttpServlet {
     ...
     */
    public InquerySearchController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		listCount = new InfoService().selectListCount(keyword);
		...
		// get방식이므로 인코딩 생략
		// Service단에서 메소드 요청
		ArrayList<Qna> list = new InfoService().selectInqSearch(pi,keyword);
		request.setAttribute("list", list);
		request.setAttribute("pi", pi);
		request.setAttribute("keyword", keyword);
		
		// 포워딩 하기
		request.getRequestDispatcher("views/admin/info/inquerySearch.jsp").forward(request, response);
