           ...
           <div class="content_title"> 매거진 조회 <hr></div>
            ...
            <!-- 페이징 바 -->
					<div id="content_4" class="paging-area" >
						<div id="page" class="page_nation" style="margin-left: 400px;" >
							<ul class="pagination" >
								<%if(currentPage==1){ %>
								<%}else{ %>
										<li class="page-item"><a class="page-link" href="/na/magSearch.mg?keyword=<%=keyword %>&currentPage=<%=currentPage - 1%>" style="color: black"><</a></li>
								<%} %>
								<%for(int p = startPage; p <= endPage; p++){ %>
									<%if(p!=currentPage){ %>
										<li class="page-item"><a class="page-link" href="/na/magSearch.mg?keyword=<%=keyword %>&currentPage=<%=p%>" style="color: black"><%=p %></a></li>
								<%} else {%>
